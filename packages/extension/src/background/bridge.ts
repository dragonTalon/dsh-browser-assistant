...
import type { BridgeCaps, ClientFrame, ServerFrame } from '@dsh-browser/protocol'
import {
...
export class BridgeClient {
  ...
  /** The bridge WebSocket URL this client is (re)connecting to. */
  get url(): string {
    return this.bridgeUrl
  }

  /** Reconnect attempt counter (0 while connected, grows during backoff). */
  ...
  private async loop(generation: number): Promise<void> {
    while (this.running && generation === this.generation) {
      ...
      const reachable = await this.probe(this.bridgeUrl).catch(() => false)
