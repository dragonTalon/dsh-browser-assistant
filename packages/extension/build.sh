...
# Resolve markdown rendering deps (marked / dompurify), same resolution:
# explicit override -> workspace install.
if [ -n "${MARKED_ESM:-}" ]; then
  : # caller-provided
elif [ -f ./node_modules/marked/lib/marked.esm.js ]; then
  MARKED_ESM=./node_modules/marked/lib/marked.esm.js
else
  echo "error: marked not found (run pnpm install --offline, or set MARKED_ESM)" >&2
  exit 1
fi
...
if [ -n "${DOMPURIFY_ESM:-}" ]; then
  ...
  echo "error: dompurify not found (run pnpm install --offline, or set DOMPURIFY_ESM)" >&2
  exit 1
fi
...
# Markdown 渲染依赖（仅 panel 用）：marked / dompurify 是本包的 devDependencies，
# alias 指向 workspace 安装的相对路径。
