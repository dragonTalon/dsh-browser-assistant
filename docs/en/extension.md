...
|---|---|---|
| **background/** | bridge client (discovery/reconnect/heartbeat), RPC forwarding and frame codec, tool dispatch, approval coordination, current-page tracking, logging | `index.ts` (assembly), `bridge.ts`, `frames.ts`, `rpc.ts`, `tools.ts`, `authorization.ts`, `approval-coordinator.ts`, `navigation.ts`, `region.ts` |
